...
function countInversions(puzzle, solved, size) {
  let res = 0;
...
function isSolvable(puzzle, solved, size) {
  solved = [].concat.apply([], solved).map((n) => parseInt(n));
  puzzle = [].concat.apply([], puzzle).map((n) => parseInt(n));
  const inversions = countInversions(puzzle, solved, size),
    ...
    taxicab =
      Math.abs(puzzle_zero_row - solved_zero_row) +
      Math.abs(puzzle_zero_column - solved_zero_column);
...
export { isSolvable, countInversions };
